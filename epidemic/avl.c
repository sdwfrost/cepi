            ...
            struct libavl_allocator *allocator)
{
  ...
  tree = (struct avl_table*)allocator->libavl_malloc (allocator, sizeof *tree);
  if (tree == NULL)
    return NULL;
...
avl_probe (struct avl_table *tree, void *item)
{
  ...
  n = q->avl_link[dir] =
    (struct avl_node*)tree->avl_alloc->libavl_malloc (tree->avl_alloc, sizeof *n);
  if (n == NULL)
    return NULL;
...
void *
avl_t_replace (struct avl_traverser *trav, void *newitem)
{
  void *olditem;

  assert (trav != NULL && trav->avl_node != NULL && newitem != NULL);
  olditem = trav->avl_node->avl_data;
  trav->avl_node->avl_data = newitem;
  return olditem;
}

static void
copy_error_recovery (struct avl_node **stack, int height,
                     struct avl_table *newtable, avl_item_func *destroy)
{
  assert (stack != NULL && height >= 0 && newtable != NULL);

  for (; height > 2; height -= 2)
    stack[height - 1]->avl_link[1] = NULL;
  avl_destroy (newtable, destroy);
}
          ...
          avl_item_func *destroy, struct libavl_allocator *allocator)
{
  ...
  struct avl_table *newtable;
  const struct avl_node *x;
  struct avl_node *y;

  assert (org != NULL);
  newtable = avl_create (org->avl_compare, org->avl_param,
                    allocator != NULL ? allocator : org->avl_alloc);
  if (newtable == NULL)
    return NULL;
  newtable->avl_count = org->avl_count;
  if (newtable->avl_count == 0)
    return newtable;

  x = (const struct avl_node *) &org->avl_root;
  y = (struct avl_node *) &newtable->avl_root;
  for (;;)
    {
      while (x->avl_link[0] != NULL)
        {
          ...
          y->avl_link[0] =
            (struct avl_node*)newtable->avl_alloc->libavl_malloc (newtable->avl_alloc,
                                           sizeof *y->avl_link[0]);
          if (y->avl_link[0] == NULL)
            {
              if (y != (struct avl_node *) &newtable->avl_root)
                {
                  y->avl_data = NULL;
                  y->avl_link[1] = NULL;
                }

              copy_error_recovery (stack, height, newtable, destroy);
              return NULL;
            }
